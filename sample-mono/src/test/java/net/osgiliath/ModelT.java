...
class ModelT {
    ...
    void givenFedModelWhenGraphBuilderIsCalledThenGraphModelEdgesArePopulated() throws Exception {
        ...
        try (GraphTraversalSource modelGraph = modelGraphBuilder.modelGraphFromMetamodelGraph(entityMetamodelGraph)) {
            ...
            assertThat(((Employee)modelGraph.V().hasLabel(Employee.class.getSimpleName()).has(MODEL_GRAPH_VERTEX_ENTITY_ID,1).out(Employee_.EMPLOYEE).values(MODEL_GRAPH_VERTEX_ENTITY).next()).getFirstName()).isEqualTo("Horace");
