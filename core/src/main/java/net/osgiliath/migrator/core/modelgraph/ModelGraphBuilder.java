...
public class ModelGraphBuilder {
    ...
    private final GraphRequester graphRequester;

    public ModelGraphBuilder(EntityImporter entityImporter, GraphTraversalSourceProvider graphTraversalSource, GraphRequester graphRequester) {
    ...
    private void createEdges(GraphTraversalSource modelGraph, org.jgrapht.Graph<MetamodelVertex, FieldEdge> entityMetamodelGraph) {
        ...
        sourceVertexEdgeAndTargetVertexList.forEach(sourceVertexEdgeAndTargetVertex ->
                sourceVertexEdgeAndTargetVertex
                        .getSourceVertex()
                        .addEdge(sourceVertexEdgeAndTargetVertex.getEdge().getFieldName(), sourceVertexEdgeAndTargetVertex.getTargetVertex())
                        .property(MODEL_GRAPH_EDGE_METAMODEL_FIELD, sourceVertexEdgeAndTargetVertex.getEdge().getMetamodelField())
        );
    }
    ...
    private Stream<SourceVertexFieldEdgeAndTargetVertex> computeEdgesOfVertices(GraphTraversalSource modelGraph, Graph<MetamodelVertex, FieldEdge> entityMetamodelGraph, List<Vertex> list) {
        Stream<SourceVertexFieldEdgeAndTargetVertex> sourceVertexEdgeAndTargetVertexList =
                list.stream().flatMap(v -> {
                        ...
                        .flatMap(edgeAndTargetVertex -> edgeAndTargetVertex.getTargetVertices().stream().map(targetVertex -> new SourceVertexFieldEdgeAndTargetVertex(edgeAndTargetVertex, targetVertex)));
        return sourceVertexEdgeAndTargetVertexList;
    }

    private static List<Vertex> allVertices(GraphTraversalSource modelGraph) {
    ...
    private Collection<Vertex> relatedVerticesOfOutgoingEdgeFromModelElementRelationship(Vertex modelVertex, FieldEdge edge, GraphTraversalSource modelGraph) {
        ...
        Optional<EdgeTargetVertexOrVertices> targetModelElementsOpt = graphRequester.getEdgeValueFromVertexGraph(modelVertex, edge, modelGraph);
        return targetModelElementsOpt.map(targetModelElements -> switch (targetModelElements) {
                    case ManyEdgeTarget m -> m.target();
                    case UnitaryEdgeTarget u -> {
